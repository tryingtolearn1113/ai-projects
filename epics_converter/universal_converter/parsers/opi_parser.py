...
class OPIParser(BaseParser):
    """Parser for CS-Studio .opi XML files."""
    ...
    def parse_file(self, filepath):
        ...
        try:
            ...
            print(f"Error parsing OPI XML: {e}")
            return [], self.screen_info

        widgets = []
        screen_info = self.screen_info.copy()
        
        # OPI files have the display dimensions at the root level
        if root.tag == "display":
            w = root.find("width")
    ...
    def _parse_widget_node(self, node):
        wtype = node.get("typeId", "Unknown")
        # Simplify typeId (e.g. org.csstudio.opibuilder.widgets.Rectangle -> Rectangle)
        wtype = wtype.split(".")[-1]
        ...
        # Scripts/Rules for visibility could be nested
